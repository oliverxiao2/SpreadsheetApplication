function ASAMFileParser(text, fileType){
  ...
  if ($FT == "A2L"){
    ...
    out = {
      ...
      */
      readCHAR: function(theCHAR){
        ...
        const $COMPU_METHOD  = this.COMPU_METHODs;
        const $RECORD_LAYOUT = this.RECORD_LAYOUTs;
        const $VTabs         = this.VTABs;
      ...
      getFuncDefAndRef: function(func, Def = true, Ref = false){
        ...
        if (FUNCs[func]){
          if (Def && FUNCs[func].DEF_CHARACTERISTIC) items = items.concat(FUNCs[func].DEF_CHARACTERISTIC.children);

          if (Ref && FUNCs[func].REF_CHARACTERISTIC) items = items.concat(FUNCs[func].REF_CHARACTERISTIC.children);
        }
      ...
      getSysConst: function () {
        const output = {};
        const node = this.getNode('MOD_PAR');
        const _keys = Object.keys(node);
        let sys, sysNode, sysconst = {};

        for (const _key of _keys) {
          if (_key != 'parent' && _key[0] === '"') {
            sys = _key; output['SYS'] = sys.replace(/\"/g, ''); break;
          }
        }

        const _props = Object.keys(node[sys]);
        const n = _props.length;

        for (let i = 0; i < n; i++) {
          if (_props[i].substr(0, 3) === 'SC_') {
            output[_props[i]] = node[sys][_props[i]];
          } 
        }
    ...
    out.CHARs = out.getNode('CHARACTERISTIC');
    out.FUNCTIONs     = out.getNode('FUNCTION');
    out.COMPU_METHODs = out.getNode('COMPU_METHOD');
    out.RECORD_LAYOUTs= out.getNode('RECORD_LAYOUT');
    out.VTABs         = out.getNode('COMPU_VTAB');
    out.TABs          = out.getNode('COMPU_TAB');
    out.SYS           = out.getSysConst();
  ...
  // parsing HEX
    ...
    for (let line of text.split('\n')) {
      ...
      if (line === '') {
        ...
        state = 'function';
        currentNode = out.functions;
        continue;
      } else if (line.match(/\[Label\]/)) {
        continue;
      } else if (line.match(/\/\*([\w]+)\*\//)) {
        state = 'label';
        const _f = line.match(/\/\*([\w]+)\*\//)[1];
        out.labels[_f] = [];
        currentNode = out.labels[_f];
        continue;
      }

      if (line && currentNode && (state === 'function' || state === 'label')) {
        currentNode.push($.trim(line));
      }
    };
  }
