...
ipcRenderer.on('selected-hex', function (event, _path) {   
    try {
        ...
        if (hex) {
            a2l.hexData = hex;
            if (a2l.readCHAR()) {
                datasetId = 'dataset-' + performance.now().toString(16).replace('.','z');
                a2lFileName = path.basename(a2lFilePath),
                hexFileName = path.basename(hexFilePath);
                $('#tab-datasets').click();
                
                if ($('#datasets-panel-a2l-hex-list:visible').length === 0) $('#datasets-panel div[data-panel=a2lhex] .insp-group-title span:first').click();
                addToDatasetsPanel($('#datasets-panel-a2l-hex-list'), datasetId, a2lFileName, hexFileName);
...
function addToDatasetsPanel (container, _id, _a, _h, _style='') {
    ...
    setTimeout(()=>{container.find('.chrome-download-item').css('opacity', 1)}, 100);
};
...
function dropSrcDesDropPanel (event) {
    ...
    if (panelIs === 'src') {
        ...
        $(target).find('.drop-panel-content').prepend(htmlTemplate);
        AppNS.destinationDataset.push(dataset);
    }
...
function whichActive ($group) {
    for (let i=0; i < $group.length; i++) {
        const btn = $($group[i]); 
        if (btn.hasClass('active')) return btn.find('.button-title').text();
    }
    return null;
};

function runDatasetCheck () {
    const group = [
        '#ribbon-btn-action-list',
        '#ribbon-btn-action-compare',
        '#ribbon-btn-action-copy'
    ];
    let action = whichActive($(group.join(',')));
    ...
    if (action === 'List') {
        if (srcDataset.type === 'A2LHEX') {
            const uniformedDFC = DSM.getDFCTable(srcDataset.data);
            const sheet = listDFCWorksheetInNewSheet(srcDataset.name[0], uniformedDFC,
            [{
                head: 'DTCO',
                prop: 'DTCO',
                width: 120,
...
function attachToolbarItemEvents () {
    ...
    $('#input-file-excel').change(function (e) {
        ...
        function excelToDFCTable (json) {
            ...
            for (const sheetname in json.sheets) {
                let recordTable = {};
                // 注意field的命名同意，例如Class的field name统一为DFESCls
                const fieldIndex = {
                    name: -1,
                    DTCO: -1,
                    FaultTyp: -1,
                ...
                let record = {}, recordname, nameColumnIndex = -1;
                ...
                for (const _c in firstRow) {
                    const fieldname = firstRow[_c].value;
                    if (fieldname.match(/DFC Name/i) || fieldname.match(/Fault P/)) fieldIndex.name = _c;
                    else if (fieldname.match(/PCODE/i)) fieldIndex.DTCO = _c;
                    else if (fieldname.match(/Three/i)) fieldIndex.FaultTyp = _c;
                    else if (fieldname.match(/Description/i)) fieldIndex.descriptionEn = _c;
                    else if (fieldname.match(/说明/)) fieldIndex.descriptionCn = _c;
                    else if (fieldname.match(/Control/i)) fieldIndex.CtlMsk = _c;
                    else if (fieldname.match(/Disable/i)) fieldIndex.DisblMsk = _c;
                    else if (fieldname.match(/Responsible/i)) fieldIndex.responsible = _c;
                ...
                for (const r of rows) {
                    recordname = sheetData[r][fieldIndex.name].value.replace(/^DFC_/, '').toUpperCase();            
                    recordTable[recordname] = {};
        
                    for (const field in fieldIndex) {
                        const cell = sheetData[r][fieldIndex[field]];
                        recordTable[recordname][field] = (cell != undefined)?((cell.value != undefined)?cell.value:''):'';
                    }
                }

                if (Object.keys(recordTable).length > 0) output.push(recordTable);
            } 
    ...
    // ***** Action - Compare *****
    // ***** Action - Copy *****
    const g = ['#ribbon-btn-action-list','#ribbon-btn-action-compare','#ribbon-btn-action-copy'];
    for (const id of g) {
        $(id).click(()=>{
            $(g.join(',')).removeClass('active');
            $(id).addClass('active');
        });
    }
    // ***** Action - Run *****
    $('#ribbon-btn-action-run').click(runDatasetCheck);
    // ***** Remove - active sheet *****
    $('#ribbon-btn-remove-active-sheet').click(removeActiveSheet);
    // ***** Remove - other sheets *****
    $('#ribbon-btn-remove-other-sheets').click(() => {if (confirm('Sure to remove other sheets?')) removeOtherSheets();});
    // ***** Remove - all sheets *****
    $('#ribbon-btn-remove-all-sheets').click(() => {if (confirm('Sure to clear all sheets?')) removeAllSheets()});
    // ***** Tab - Main End *****
    // ***** Tab - Data *****
    ...
    $('#ribbon-btn-add-sheet-of-DTC-list').click(() => {
        ...
        if (srcDataset.type === 'A2LHEX') {
            ...
            const srcMILDef = srcDataset.data.CHARACTERISTIC.DFES_xClsFltMIL_CA.phyDec,
                  srcSVSDef = srcDataset.data.CHARACTERISTIC.DFES_xClsFltSVS_CA.phyDec;
            const fields = [{
                    head: 'Description CN',
                    prop: 'descriptionCn',
                    width: 180,
            ...
            const unionDFC = Object.assign(srcDFC);
            for (const di of desDatasets) {
                if (di.type === 'EXCEL') {
                    const desDFC = di.data[0];
                    fields.push({
                        head: di.name,
                        prop: di.name,
                        width: 120,
                    ...
                    for (const DFCName in unionDFC) {
                        if (desDFC[DFCName]) {
                            unionDFC[DFCName].MIL = readLamp(parseInt(unionDFC[DFCName].DFESCls), srcMILDef, is1788, DFCName);
                            unionDFC[DFCName].SVS = readLamp(parseInt(unionDFC[DFCName].DFESCls), srcSVSDef, is1788, DFCName);
                            unionDFC[DFCName].descriptionCn = desDFC[DFCName].descriptionCn;
                            unionDFC[DFCName].descriptionEn = desDFC[DFCName].descriptionEn;
                            unionDFC[DFCName][di.name] = '\u2713';
                        } else {
                            unionDFC[DFCName][di.name] = '';
                        }
                    }
            ...
            console.log(unionDFC, fields);
            listDFCWorksheetInNewSheet('DTC List_' + srcDataset.name[0], unionDFC, fields);
        }

        function readLamp (Cls, Def, is1788 = false, DFCName = '') {
            if (Def.length > 0 && Cls >= 0) {
                if (typeof Def[Cls] === 'string') {
                    if (Cls === 2) {
                        if (is1788 && (!(DFCName.match(/max/i)))) return '\u2713';
                        else return 'o => \u2713';
                    } else {
                        if (Def[Cls].match('NO_')) return '\u2717'; //
                        else if (Def[Cls].match('Blinking')) return 'o'; //
    ...
    srcDatasetDropPanel.addEventListener('dragover', (event) => {event.preventDefault();});
    srcDatasetDropPanel.addEventListener('drop', dropSrcDesDropPanel);

    desDatasetDropPanel.addEventListener('dragover', (event) => {event.preventDefault();});
...
function listDFCWorksheetInNewSheet (_sheetname, _DFCInfoList, fields) {
    ...
    const startRow = 2,
          startColumn = 2,
          maxRowCount = startRow + names.length + 3,
          maxColCount = startColumn + fields.length + 4,
          packLen = 60;
    ...
    for (let i=0; i < names.length; i++) {
        name = names[i];
        DFC  = _DFCInfoList[name];
        packDataRow = [];

        // head
        if (i === 0) {
            sheet.setValue(startRow, startColumn, '#');
            sheet.setValue(startRow, startColumn+1, 'DFC Name');
        }
        
        sheet.setValue(startRow+1+i, startColumn, i+1);
        sheet.setValue(startRow+1+i, startColumn+1, DFC.name);
        ...
        if (i % packLen === (packLen - 1) || i === (names.length -1)) {
            ...
            setTimeout(()=>{
                fillTask(startRow+1+i - (duplicate.length-1), startColumn+2, duplicate);
            }, 0);
            packDataList = [];
        } 
    ...
    setSheetStyle({
        sheet: sheet,
        style: 'defaultDarkBlue',
        fieldCount: fields.length +2,
    ...
    function fillTask(startRow, startColumn, list) {
        spread.suspendPaint();
...
function setSheetStyle ({
    ...
    const {
        ...
        paperColor,
        defaultFont,
        titleForeColor} = sheetStylePresets[style];
    spread.suspendPaint();
    // 将整个表单区域上色pageColor,并修改默认字体
    sheet.getRange(0, 0, maxRowCount, maxColCount).backColor(paperColor).font(defaultFont);
    
    // page margin区域上色backColor1
    sheet.getRange(0, 0, 1, maxColCount).backColor(backColor1);
    sheet.getRange(0, 0, maxRowCount, 1).backColor(backColor1);
    sheet.getRange(0, maxColCount-1, maxRowCount, 1).backColor(backColor1);
    sheet.getRange(maxRowCount-1, 0, 1, maxColCount).backColor(backColor1);

    sheet.getRange(startRow, startColumn, 1, fieldCount).backColor(backColor2).foreColor(titleForeColor)
    sheet.getRange(startRow, startColumn, maxRowCount - startRow, maxColCount - startColumn).hAlign(spreadNS.HorizontalAlign.center);
    sheet.getRange(startRow, startColumn+1, maxRowCount - startRow, 1).hAlign(spreadNS.HorizontalAlign.left);

    // 调整列宽
    for (let i = 0; i < maxColCount; i++) {
        if (i >=4 && fields[i-4] && fields[i-4].width >0) {
            sheet.setColumnWidth(i, fields[i-4].width);
        } else if (columnWidthSetting[i] === undefined) {
            if (columnWidthSetting.autofit) sheet.autoFitColumn(i);
            else sheet.setColumnWidth(i, columnWidthSetting.default);
        } else sheet.setColumnWidth(i, columnWidthSetting[i]);
    }

    // 调整行高
    for (let j = 0; j < rowHeightSetting.length; j++) {
        sheet.setRowHeight(j, rowHeightSetting[j]);
    }
...
function addSheet (_sheetname, _rowCount = 9999, _columnCount = 26, show = false) {
    const timeStamp = performance.now().toString(16).replace('.', 'z');
    if (spread.getSheetFromName(_sheetname)) _sheetname += timeStamp;
    
    const sheet = new GC.Spread.Sheets.Worksheet(_sheetname);
    sheet.setRowCount(_rowCount);
    sheet.setColumnCount(_columnCount);
    const sheetCount = spread.getSheetCount();
    spread.addSheet(sheetCount, sheet);
    if (show) spread.setActiveSheet(_sheetname);
    return sheet;
};

function removeActiveSheet () {
    const n = spread.getActiveSheetIndex();
    if (n != -1) spread.removeSheet(n);
};

function removeOtherSheets () {
    const n = spread.getSheetCount();
    const k = spread.getActiveSheetIndex();
    let j = 0;
    for (let i = 0; i < n; i++) {
        if (j >= n) return;
        else if (i != k) spread.removeSheet(j);
        else j++;
    }
};
...
function updateLabelCheckIconState () {
    ...
    const g = ['#ribbon-btn-label-dfc-list', '#ribbon-btn-label-dsm-scantool', '#ribbon-btn-label-power-stage'];
    if (d) {
